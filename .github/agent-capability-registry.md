...
**Default flow:** Strategist → Sparring → UX → UserStory → Architect → DataAPI → PM → RK_Coder → Tester → Reviewer → Security → Ops → Release → Docs → Knowledge Curator  
...
**Primary handoffs:** DataAPI (contracts), Security (threats), Ops (operability), RK_Coder (implementation handoff), Reviewer (design review).
...
**Primary handoffs:** RK_Coder (implement contract), Tester (contract test design), Security (auth/PII), Architect (boundary alignment).

> Note: Contract artifacts are allowed as “interface specifications” but should be treated as non-implementation. If your governance wants **zero** code-block-like artifacts here, route spec generation to RK_Coder instead.
...
- design systems
- write code / code blocks
**Primary handoffs:** Strategist (priorities), Architect (design), RK_Coder (implementation request), Tester/Reviewer/Security/Ops/Release/Docs.
...
**Primary handoffs:** RK_Coder (implement tests), Reviewer (coverage review), Security/Ops (special scenarios).
...
- output patch diffs
- write code / code blocks
**Primary handoffs:** RK_Coder (fix findings), Tester (add scenarios), Security (deep risk), Architect (design deviation).
...
- implement fixes
- write code / code blocks
**Primary handoffs:** RK_Coder (implement mitigations), Tester (security scenarios), Ops (hardening), Compliance (audit controls), Architect (boundary changes).
...
- write code / code blocks
**Primary handoffs:** RK_Coder (implement optimizations/tests), Architect (systemic changes), Ops (infra tuning), Tester (perf validation design).
...
- write code / code blocks
**Primary handoffs:** RK_Coder (implement configs), Security/Compliance (controls), Tester (smoke scenarios), Release (rollout).
...
- write code / code blocks
**Primary handoffs:** Ops (ops readiness), Tester (release validation), PM (schedule/comms), RK_Coder (automation/config), Documentation/Knowledge Curator.
...
- write code / code blocks
**Primary handoffs:** RK_Coder (implement tooling/config), Ops (CI parity), Documentation.
...
- provide implementation snippets
- write code / code blocks
**Primary handoffs:** DataAPI (contract), Security (auth/PII), Ops (operability signals), Tester (validation design), RK_Coder (implementation).
...
- write code / code blocks
**Primary handoffs:** Architect (boundaries), DataAPI (contract changes), Tester (regression design), Ops (rollout readiness), PM (timeline), RK_Coder (implementation).
...
**Primary handoffs:** Architect (validate design), DataAPI (contract references), RK_Coder (if code examples are required), Knowledge Curator (canonize).
...
- write code / code blocks
**Primary handoffs:** Knowledge Curator (store taxonomy), Researcher (validate domain), Architect (system design), Integration (tooling strategy), RK_Coder (implementation).
...
- output pseudo-code
- write code / code blocks
**Primary handoffs:** Architect (scaling implications), RK_Coder (implement derived logic), Strategist (quantitative decisions), Performance (validate).
...
- Handoff prompt for RK_Coder
**Must NOT:**
- output completions/snippets
- write code / code blocks
**Primary handoffs:** RK_Coder (implementation).
